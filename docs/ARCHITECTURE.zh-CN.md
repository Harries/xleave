...
| 管理后台 | Express 服务端渲染、签名 HttpOnly Cookie |
...
Authorization: Bearer <该用户的 Token>
```
...
公开的公网 IP 探测接口。它只返回 Vercel 从请求中识别到的 `x-real-ip`，用于用户填写自己的 `allowedIps`，不会调用 OpenAI 或修改用户配置。
...
服务端渲染的管理员后台。管理员使用 `XLEAVE_ADMIN_TOKEN` 登录后，可以：
...
### 7.1 多用户与 Bearer Token

文件：`server/src/auth.js`

正式用户数据存储在 Neon Postgres。旧版环境变量继续作为迁移兜底：
...
- 服务端没有有效用户配置：返回 `503`。
- Token 缺失或找不到对应用户：返回 `401`。
- Token 正确：后端自动识别用户，并把用户信息传给 IP 校验。

后端先计算 Token 的 SHA-256：Neon 用户通过唯一哈希索引查找，环境变量兜底用户使用定时安全哈希比较。

Neon 中不保存 Token 明文，而是保存 SHA-256 哈希与 Token 尾号。通过环境变量兜底的旧用户仍包含明文 Token，因此应尽快迁移到后台。

### 7.2 公网 IP 白名单

文件：`server/src/ip-access.js`

白名单来自已认证用户记录的 `allowedIps`。
...
每次通过 Token 校验并进入 IP 校验的请求都会把用户 ID 和检测到的公网 IP 写入 Vercel Function 日志。IP 被拒绝时，`403` 响应也会返回该 IP，方便更新该用户白名单。
...
- 只有接口地址的第三方无法调用。
- 只有某用户 Token、但不在该用户许可网络中的第三方无法调用。
- 只有许可网络、但没有对应用户 Token 的设备无法调用。
- 单个用户可独立轮换或删除，不影响其他用户。
...
```json
{
  "source": {
    ...
    "handle": "@handle",
    "text": "原帖正文",
    "languageHint": "插件检测到的原帖语言提示，例如 Simplified Chinese / Japanese / the same Latin-script language used by the source post",
...
- 使用原帖主要语言，或使用用户指定语言。
- 当 `preferences.language` 为 `auto` 时，插件会先根据原帖正文给出 `source.languageHint`，后端要求模型按原帖主语言输出；如果用户在插件设置中手动选择了中文、英文或日文，则以用户设置为准。
- 遵守最大字符数。
- 不编造事实、经历、关系或承诺。
