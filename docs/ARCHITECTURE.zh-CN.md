...
3. 用户点击按钮后，插件提取原帖、作者和可见上下文。
4. 插件通过 HTTPS 调用 Express 后端。
5. 后端完成 Token 与公网 IP 双重校验。
6. 后端调用 OpenAI Responses API，生成三条不同风格的文本回复。
...
X 页面可能同时存在多个 `article`。插件会：

1. 收集弹窗、当前文章和主内容区域中可见的文章。
2. 去重并排除不可见元素。
3. 根据文章和回复框的屏幕位置计算距离。
4. 优先选择回复框上方距离最近的文章作为原帖。
5. 将其他可见帖子作为可选上下文。

这是一套 DOM 启发式算法，不依赖 X 官方 API。
